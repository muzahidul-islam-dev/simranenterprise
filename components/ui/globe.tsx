...
export function Globe({ globeConfig, data }: WorldProps) {
  ...
  const defaultProps = {
    ...
    arcTime: 2000,
    arcLength: 0.9,
    rings: 0,       // DISABLED — rings are expensive
    maxRings: 0,
    ...globeConfig,
  };
  ...
  useEffect(() => {
    ...
    // Rings completely disabled
    globeRef.current.ringsData([]);
  }, [isInitialized, data]);

  // No ring interval — removed entirely
...
export function WebGLRendererConfig() {
  ...
  useEffect(() => {
    // Hard cap at 1 on mobile, 1.5 on desktop — biggest single performance win
...
function FrameController({ canvasRef }: { canvasRef: React.RefObject<HTMLDivElement | null> }) {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        // Pause WebGL loop when not visible
